...
crypto_sign_keypair(
    ...
    keygen(&rng, f, g, F, NULL, h, 10, tmp.b);
...
do_sign(uint8_t *nonce, uint8_t *sigbuf, size_t *sigbuflen,
    ...
    if (!complete_private(G, f, g, F, 10, tmp.b)) {
        return -1;
    }
    ...
    hash_to_point_ct(&sc, r.hm, 10, tmp.b);
     ...
     */
    for (;;) {
        sign_dyn(r.sig, &sc, f, g, F, G, r.hm, 10, tmp.b);
        v = comp_encode(sigbuf, *sigbuflen, r.sig, 10);
        if (v != 0) {
            inner_shake256_ctx_release(&sc);
            *sigbuflen = v;
            return 0;
...
do_verify(
    ...
    if (comp_decode(sig, 10, sigbuf, sigbuflen) != sigbuflen) {
        return -1;
    }
    ...
    inner_shake256_inject(&sc, m, mlen);
    inner_shake256_flip(&sc);
    hash_to_point_ct(&sc, hm, 10, tmp.b);
    inner_shake256_ctx_release(&sc);

    /*
     * Verify signature.
     */
    if (!verify_raw(hm, sig, h, 10, tmp.b)) {
        return -1;
    }
    return 0;
}
