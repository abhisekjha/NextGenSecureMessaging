...
static void GenMatrix(polyvec *a, const unsigned char *seed) {
    ...
    for (i = 0; i < SABER_K; i++) {
        for (j = 0; j < SABER_K; j++) {
            BS2POL(buf + (i * SABER_K + j) * (13 * SABER_N / 8), temp_ar);
            for (k = 0; k < SABER_N; k++) {
...
void indcpa_kem_enc(const unsigned char *message_received, unsigned char *noiseseed, const unsigned char *pk, unsigned char *ciphertext) {
    uint32_t i, j, k;
    polyvec a[SABER_K];
    unsigned char seed[SABER_SEEDBYTES];
