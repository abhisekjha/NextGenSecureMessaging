...
int rainbow_sign_cyclic(uint8_t *signature, const csk_t *csk, const uint8_t *digest) {
    ...
    generate_secretkey_cyclic((sk_t *)sk, csk->pk_seed, csk->sk_seed); // generating classic secret key.
    return rainbow_sign(signature, (sk_t *)sk, digest);
}

int rainbow_verify_cyclic(const uint8_t *digest, const uint8_t *signature, const cpk_t *_pk) {
    unsigned char pk[sizeof(pk_t) + 32];
    cpk_to_pk((pk_t *)pk, _pk); // generating classic public key.
    return rainbow_verify(digest, signature, (pk_t *)pk);
}
