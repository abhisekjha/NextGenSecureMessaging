 ...
 */
unsigned long long bytes_to_ull(
...
void compute_root(
    ...
    for (i = 0; i < tree_height - 1; i++) {
        ...
        if (leaf_idx & 1) {
            thash_2(
                buffer + N, buffer, pub_seed, addr, hash_state_seeded);
            memcpy(buffer, auth_path, N);
        } else {
            thash_2(
 ...
 * tree type (i.e. ADDR_TYPE_HASHTREE or ADDR_TYPE_FORSTREE).
...
static void treehash(
    ...
    for (idx = 0; idx < (uint32_t)(1 << tree_height); idx++) {
        ...
        while (offset >= 2 && heights[offset - 1] == heights[offset - 2]) {
            ...
            thash_2(
                stack + (offset - 2)*N, stack + (offset - 2)*N,
...
void treehash_FORS_HEIGHT(
    ...
    unsigned char stack[(FORS_HEIGHT + 1)*N];
    unsigned int heights[FORS_HEIGHT + 1];

    treehash(
        root, auth_path, stack, heights, sk_seed, pub_seed,
        leaf_idx, idx_offset, FORS_HEIGHT, gen_leaf, tree_addr, hash_state_seeded);
}

void treehash_TREE_HEIGHT(
    unsigned char *root, unsigned char *auth_path,
    const unsigned char *sk_seed, const unsigned char *pub_seed,
    uint32_t leaf_idx, uint32_t idx_offset,
    void (*gen_leaf)(
    ...
    unsigned char stack[(TREE_HEIGHT + 1)*N];
    unsigned int heights[TREE_HEIGHT + 1];

    treehash(
        root, auth_path, stack, heights, sk_seed, pub_seed,
        leaf_idx, idx_offset, TREE_HEIGHT, gen_leaf, tree_addr, hash_state_seeded);
}
