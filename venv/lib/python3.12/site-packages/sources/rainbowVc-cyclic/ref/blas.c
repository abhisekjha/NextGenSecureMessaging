...
void gf256v_mul_scalar(uint8_t *a, uint8_t b, size_t _num_byte) {
    for (size_t i = 0; i < _num_byte; i++) {
        a[i] = gf256_mul(a[i], b);
    }
}

void gf256v_madd(uint8_t *accu_c, const uint8_t *a, uint8_t gf256_b, size_t _num_byte) {
    for (size_t i = 0; i < _num_byte; i++) {
        accu_c[i] ^= gf256_mul(a[i], gf256_b);
    }
}
